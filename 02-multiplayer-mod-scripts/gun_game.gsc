...
Init()
{
    // yourserver.gg: DISABLED 2026-06-28. Gun Game now runs on the NATIVE Plutonium "gun"
    // gametype via a loose override at maps/mp/gametypes/gun.gsc (random 30-gun pool,
    // level.matchrules_randomize = 1). The native gametype owns the weapon-progression
    // HUD counter, the live score, and the win at scr_gun_scorelimit (= 30) — all of
    // which this Resxt overlay was fighting (frozen 1/18 counter, score only refreshing
    // on alt-tab, game never ending). Leaving the rest of this file inert for reference.
    return;
...
InitConfigVariables()
{
    ...
    level.gun_game_weapons_amount = 30; // yourserver.gg: weapon count == kills-to-win (each kill = next gun, last gun's kill wins) => 30 = FIRST TO 30 KILLS WINS. Must stay <= 49 usable base weapons (unique_check "weapon") or InitCurrentGameWeapons infinite-loops = "Setting up game" hang. 30 is safe.
...
InitWeaponVariables()
{
    ...
    level.gun_game_available_weapons["secondary_weapons"]["launchers"] = ["iw5_smaw_mp", "javelin_mp", "xm25_mp", "m320_mp"]; // yourserver.gg: rpg_mp removed - it doesn't exist in IW5 (invisible hands); stinger also removed (can't shoot players)
...
GetRandomWeapon()
{
    ...
    // yourserver.gg: give shotguns PLAIN (no attachment, no camo). The script gives shotguns a
    // scope 100% of the time, but the lever-action 1887 (and some others) has no optic
    // rail, so weapon+scope = an asset that doesn't exist = client CRASH on spawn. Plain
    // base shotguns are always valid. Return early so no attachment/camo is appended.
    if (GetWeaponClass(randomWeapon) == "weapon_shotgun")
    {
        return randomWeapon;
    }
...
OnPlayerKill()
{
    ...
    for (;;)
    {
        ...
        // yourserver.gg: score == real progression (kills, 0..amount) so the bottom-left
        // counter adds up; and END THE GAME OURSELVES when a player finishes the last
        // gun. The "gun" gametype did not honor the old self.score/scorelimit hack
        // (35 kills never ended the match), so we win explicitly via endGame().
        self.score = self.pers["gun_game_current_index"];
        self.pers["score"] = self.score;

        if (self.pers["gun_game_current_index"] >= level.gun_game_weapons_amount)
        {
            self.pers["gun_game_current_index"] = (level.gun_game_weapons_amount - 1); // clamp HUD to amount/amount
            level thread maps\mp\gametypes\_gamelogic::endGame( self, game["strings"]["score_limit_reached"] );
        }
        else
        {
...
OnPlayerReload()
{
    ...
    for (;;)
    {
        ...
        if ( isDefined( self.pers["gun_game_current_weapon"] ) )
...
ClearPlayerClass()
{
    // yourserver.gg: build a fully-stripped loadout WITHOUT cloneLoadout(). cloneLoadout()
    // reads self.curClass / level.classMap[class]; for bots and any spawn whose class
    // index is undefined it floods the console with class-table lookup errors on every
    // (re)spawn. Gun game hands out its own weapon + perks right after this, so all we
    // need is an empty loadout built directly with the sentinel values that
    // giveLoadout("gamemode") accepts ("specialty_null" for buffs/streaktype, "none"
    // elsewhere). No dependency on the class system, so it is safe for bots too.
    loadout = [];
    loadout["inUse"] = false;
