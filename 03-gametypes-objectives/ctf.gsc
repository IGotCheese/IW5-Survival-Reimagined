...
getspawnpoint()
{
    ...
    {
        // getspawnpointarray (not raw getentarray) so script-injected spawns in
        // level.extraspawnpoints are seen during grace too - maps like mp_gulag ship
        // no classname spawn ents, so getentarray returns empty here and the spawn
        // select falls through undefined (bots spawn with no self.spawnpos -> the
        // logplayerlife matchdata error on every death).
...
// ===== mp_boomtown CTF fix (2026-06-28) ============================================
// The map ships its CTF flags, zones and spawns dumped in an off-map corner ~800u
// under the floor (allies/axis flags only 128u apart). Relocate them onto the real
// playable area (the working DOM-flag ends) at opposite ends, and inject extra
// on-map spawns (map ships only 1/team). Runs first in onstartgametype, before the
// spawn placement (line ~149) and the flag setup (ctf() -> createteamflag/capzone)
// read the entity origins. ctf.gsc rebuilds the capture trigger from ctf_zone.origin,
// so moving the zone origin moves the actual objective.
ctf_fixboomtown()
{
    if ( level.script != "mp_boomtown" )
        return;

    var_0 = ( 0.0, -2150.0, 833.0 );
    var_1 = ( 0.0, 2150.0, 833.0 );

    ctf_moveent( "ctf_flag_allies", var_0, 90 );
    ctf_moveent( "ctf_zone_allies", var_0, 90 );
    ctf_moveent( "ctf_trig_allies", var_0, 90 );
    ctf_moveent( "ctf_flag_axis", var_1, 270 );
    ctf_moveent( "ctf_zone_axis", var_1, 270 );
    ctf_moveent( "ctf_trig_axis", var_1, 270 );

    ctf_movespawns( "mp_ctf_spawn_allies", var_0, 90 );
    ctf_movespawns( "mp_ctf_spawn_allies_start", var_0, 90 );
    ctf_movespawns( "mp_ctf_spawn_axis", var_1, 270 );
    ctf_movespawns( "mp_ctf_spawn_axis_start", var_1, 270 );

    if ( !isdefined( level.extraspawnpoints ) )
        level.extraspawnpoints = [];

    level.extraspawnpoints[ "mp_ctf_spawn_allies" ] = ctf_spawnfan( var_0, 1, 90 );
    level.extraspawnpoints[ "mp_ctf_spawn_allies_start" ] = ctf_spawnfan( var_0, 1, 90 );
    level.extraspawnpoints[ "mp_ctf_spawn_axis" ] = ctf_spawnfan( var_1, -1, 270 );
    level.extraspawnpoints[ "mp_ctf_spawn_axis_start" ] = ctf_spawnfan( var_1, -1, 270 );

    logprint( "CTF_OVERRIDE: relocated mp_boomtown CTF objectives + spawns to map ends\n" );
}

ctf_fixmeleeresort()
{
    if ( level.script != "mp_melee_resort" )
        return;

    var_0 = ( 689.0, -1038.0, 768.0 );
    var_1 = ( 641.0, 1056.0, 768.0 );
    ...
    logprint( "CTF_OVERRIDE: relocated mp_melee_resort CTF objectives + spawns\n" );
}

// mp_gulag ships NO CTF content at all (no flags, no zones, no team spawns), so CTF
// falls back to FFA. Build the two flags + zones and inject team spawns via
// level.extraspawnpoints (which _spawnlogic::getspawnpointarray merges into
// placespawnpoints/addspawnpoints). Must run BEFORE placespawnpoints (line ~152) and
// before ctf() (line ~171). Guarded to this map + one-shot. Coordinates hand-placed.
ctf_fixgulag()
{
    if ( level.script != "mp_gulag" )
        return;

    if ( isdefined( getent( "ctf_flag_allies", "targetname" ) ) )
        return;

    var_0 = ( 132.643, -403.535, 16.125 );
    var_1 = ( 1516.8, -410.356, 16.125 );

    // Flags + zones. createteamflag reads ctf_flag_<team> (script_model) + reads
    // ctf_zone_<team>.origin/.height; createcapzone uses ctf_zone_<team> as the
    // (instant, usetime 0) capture trigger.
    ctf_make_flag( "allies", var_0, -4 );
    ctf_make_flag( "axis", var_1, 76 );

    // Team spawns - allies face +x toward axis (yaw 0), axis face -x (yaw 180).
    if ( !isdefined( level.extraspawnpoints ) )
        level.extraspawnpoints = [];

    level.extraspawnpoints["mp_ctf_spawn_allies"] = ctf_spawnfan( var_0, 1, 0 );
    level.extraspawnpoints["mp_ctf_spawn_allies_start"] = ctf_spawnfan( var_0, 1, 0 );
    level.extraspawnpoints["mp_ctf_spawn_axis"] = ctf_spawnfan( var_1, -1, 180 );
    level.extraspawnpoints["mp_ctf_spawn_axis_start"] = ctf_spawnfan( var_1, -1, 180 );

    logprint( "CTF_OVERRIDE: spawned mp_gulag CTF flags + zones + spawns\n" );
}

// mp_bo2frost (custom BO2 port) ships NO CTF content, so createteamflag/createcapzone
// return undefined and the Bot Warfare bot_cap loop floods istouching/distancesquared
// errors on every bot. Build the two flags + zones + inject team spawns, exactly like
// ctf_fixgulag. Bases are the two farthest Bot Warfare waypoint ends (long X axis):
// allies at -x, axis at +x, ~5400u apart. Runs BEFORE placespawnpoints + ctf().
ctf_fix_bo2frost()
{
    if ( level.script != "mp_bo2frost" )
        return;

    if ( isdefined( getent( "ctf_flag_allies", "targetname" ) ) )
        return;

    var_0 = ( -2611.0, -453.0, 65.0 );
    var_1 = ( 2570.0, 291.0, -11.0 );

    ctf_make_flag( "allies", var_0, 0 );
    ctf_make_flag( "axis", var_1, 180 );
    ...
    logprint( "CTF_OVERRIDE: spawned mp_bo2frost CTF flags + zones + spawns\n" );
}

// Spawn one team's flag (script_model; createteamflag sets its model) and its zone
// (trigger_radius; .height is read by createteamflag). Same targetnames the gametype
// looks up via getent.
ctf_make_flag( var_0, var_1, var_2 )
{
    var_3 = spawn( "script_model", var_1 );
    var_3.angles = ( 0.0, var_2, 0.0 );
    var_3.targetname = "ctf_flag_" + var_0;

    var_4 = spawn( "trigger_radius", var_1, 0, 96, 128 );
    var_4.angles = ( 0.0, var_2, 0.0 );
    var_4.targetname = "ctf_zone_" + var_0;
    var_4.height = 128;
}

ctf_moveent( var_0, var_1, var_2 )
{
...
ctf_movespawns( var_0, var_1, var_2 )
{
    var_3 = getentarray( var_0, "classname" );

    for ( var_4 = 0; var_4 < var_3.size; var_4++ )
    {
        var_3[var_4].origin = var_1;
        var_3[var_4].angles = ( 0.0, var_2, 0.0 );
    }
}

// Fresh script_origin spawn copies clustered toward map center from a base. var_1 is
// the toward-center sign for Y (+1 allies / -1 axis); var_2 is the facing yaw.
ctf_spawnfan( var_0, var_1, var_2 )
{
    var_3 = [];
    var_4 = [];
    var_4[0] = ( 0.0, 100.0, 0.0 );
    var_4[1] = ( 90.0, 60.0, 0.0 );
    var_4[2] = ( -90.0, 60.0, 0.0 );
    var_4[3] = ( 90.0, 150.0, 0.0 );
    var_4[4] = ( -90.0, 150.0, 0.0 );

    for ( var_5 = 0; var_5 < var_4.size; var_5++ )
    {
        var_6 = ( var_4[var_5][0], var_4[var_5][1] * var_1, var_4[var_5][2] );
        var_7 = spawn( "script_origin", var_0 + var_6 );
        var_7.angles = ( 0.0, var_2, 0.0 );
