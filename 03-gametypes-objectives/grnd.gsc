...
initfirstzone()
{
    // Define the per-team in-zone counter up front (before level.grnd_zone is
    // created below) so the bots mod, which gates on level.grnd_zone then reads
    // level.grnd_numplayers, can never see it undefined. grnd_trackzonecounts()
    // keeps it current.
    if ( !isdefined( level.grnd_numplayers ) )
        level.grnd_numplayers = [];

    level.grnd_numplayers[ "allies" ] = 0;
    level.grnd_numplayers[ "axis" ] = 0;

    // Hand-placed mp_poolparty drop zones. Forced here (after createzones, before the
    // auto-gen guard) so these exact spots win over any grnd_dropZone entity or the
    // auto-generator. Add/edit entries to change the zones.
    if ( level.script == "mp_poolparty" )
    {
        level.grnd_dropzones[ "mp_poolparty" ] = [];
        level.grnd_dropzones[ "mp_poolparty" ][0] = ( 716.0, 624.0, 0.0 );
        level.grnd_dropzones[ "mp_poolparty" ][1] = ( -781.0, 640.0, 0.0 );
        level.grnd_dropzones[ "mp_poolparty" ][2] = ( 6.0, 705.0, 0.0 );
    }

    var_needzones = 0;

    if ( !isdefined( level.grnd_dropzones[level.script] ) )
        var_needzones = 1;
    else if ( level.grnd_dropzones[level.script].size < 2 )
        var_needzones = 1;

    if ( var_needzones )
    {
        level.grnd_dropzones[level.script] = grnd_autozones();
        logprint( "GRND_OVERRIDE: auto-generated " + level.grnd_dropzones[level.script].size + " drop zones for " + level.script + "\n" );
    }
    ...
    level thread grnd_trackzonecounts();
}
...
randomdrops()
{
    ...
    for (;;)
    {
        ...
        // yourserver.gg: custom maps have no care_package/airdrop_crate entity, so
        // _airdrop::init never sets level.airdropcratecollision -> createairdropcrate's
        // clonebrushmodeltoscriptmodel(undefined) floods the log every drop on Drop Zone.
        // Skip the bonus care-package drop when there's no collision brush; the zone-
        // capture objective (the actual DZ gameplay) is unaffected.
        if ( isdefined( var_0 ) && isdefined( level.airdropcratecollision ) && maps\mp\_utility::currentactivevehiclecount() < maps\mp\_utility::maxvehiclesallowed() && level.fauxvehiclecount + var_1 < maps\mp\_utility::maxvehiclesallowed() && level.numdropcrates < 8 )
        {
...
// ===== Your Server additions: make Drop Zone (grnd) work on ANY map =====
// Stock grnd.gsc hard-codes drop-zone coords per map + needs sab_bomb and
// mp_sab_spawn_*_start; custom maps have none -> undefined -> crash-loop.
// These helpers synthesize that data from the map's own spawn points.

grnd_collectspawnorigins()
{
    var_0 = maps\mp\gametypes\_spawnlogic::getspawnpointarray( "mp_tdm_spawn" );

    if ( !var_0.size )
        var_0 = getentarray( "mp_dm_spawn", "classname" );

    if ( !var_0.size )
        var_0 = getentarray( "mp_dom_spawn", "classname" );
...
grnd_ensurestartspawns()
{
    // Build a pool of usable spawn locations from whatever this map actually has.
    var_0 = getentarray( "mp_tdm_spawn", "classname" );
    ...
    if ( !var_0.size )
        var_0 = getentarray( "mp_sab_spawn", "classname" );
    ...
    // (A) TEAM spawns. grnd fills level.spawnpoints via addspawnpoints(team,"mp_tdm_spawn").
    // If the map has no real mp_tdm_spawn ents, inject NEW spawn-point copies so that
    // array is non-empty -- otherwise _spawnlogic::spawnperframeupdate loops over an
    // empty level.spawnpoints (% 0 -> OP_mod error) and spawnpointupdate() runs on
    // undefined, flooding the log every frame and hanging the match.
    if ( !getentarray( "mp_tdm_spawn", "classname" ).size )
    {
        var_1 = [];

        for ( var_2 = 0; var_2 < var_0.size; var_2++ )
            var_1[var_1.size] = grnd_clonespawn( var_0[var_2] );

        level.extraspawnpoints[ "mp_tdm_spawn" ] = var_1;
    }

    // (B) GRACE-period start spawns. If the map lacks mp_sab_spawn_*_start, inject NEW
    // copies (split allies/axis). CRITICAL: these are SEPARATE clones, never the
    // mp_tdm_spawn ents themselves -- placespawnpoints() inits whatever it is given,
    // and an already-inited tdm ent is then SKIPPED by addspawnpoints(), which is the
    // exact bug that emptied level.spawnpoints and caused the flood.
    if ( getentarray( "mp_sab_spawn_allies_start", "classname" ).size && getentarray( "mp_sab_spawn_axis_start", "classname" ).size )
        return;
    ...
    {
        if ( var_5 % 2 == 0 )
            var_3[var_3.size] = grnd_clonespawn( var_0[var_5] );
        else
            var_4[var_4.size] = grnd_clonespawn( var_0[var_5] );
    }
    ...
    level.extraspawnpoints[ "mp_sab_spawn_allies_start" ] = var_3;
    level.extraspawnpoints[ "mp_sab_spawn_axis_start" ] = var_4;
}

// Make a fresh script_origin spawn-point at the same spot/orientation as a source
// spawn entity, so we can register it for a grnd spawn class WITHOUT pre-initing
// (and thereby excluding) the map's real spawn entities. See grnd_ensurestartspawns.
grnd_clonespawn( var_0 )
{
...
grnd_autozones()
{
    var_0 = grnd_collectspawnorigins();

    if ( var_0.size < 2 )
        return grnd_fallbackzones( var_0 );
    ...
    while ( var_1.size < 10 )
    {
        ...
        for ( var_4 = 0; var_4 < var_0.size; var_4++ )
        {
            ...
            {
                var_7 = distance2d( var_0[var_4], var_1[var_6] );
...
// Defensive fallback: a map with fewer than 2 usable spawn origins would make
// grnd_autozones() return an empty/undersized list, and initfirstzone() would
// then index [0] on it -> undefined -> spawn() crash (whole-server crash-loop,
// because the bots mod links grnd on every map). Guarantee >=2 distinct zones
// around the computed map center so the gametype degrades gracefully instead.
grnd_fallbackzones( var_0 )
{
    var_1 = [];
    ...
    var_4 = [];
    var_4[0] = ( 384.0, 384.0, 0.0 );
    var_4[1] = ( -384.0, -384.0, 0.0 );
    var_4[2] = ( 384.0, -384.0, 0.0 );
    var_4[3] = ( -384.0, 384.0, 0.0 );

    for ( var_5 = 0; var_5 < var_4.size && var_1.size < 4; var_5++ )
        var_1[var_1.size] = var_3 + var_4[var_5];

    return var_1;
}

// Stock grnd maintains level.grnd_numplayers[team] = alive players currently in the
// active drop zone, per team. The decompiled gametype dropped it, but the bots mod
// (_bot_script.gsc::bot_grnd_loop) reads it every loop -> 'undefined' index/compare
// errors that spam the log and stop bots playing the objective. Rebuild it on a
// light timer from the per-player .ingrindzone flag this file already maintains.
grnd_trackzonecounts()
{
    level endon( "game_ended" );
    ...
    for ( ;; )
    {
        ...
        foreach ( var_2 in level.players )
        {
            ...
            if ( !isdefined( var_2.ingrindzone ) || !var_2.ingrindzone )
                continue;
            ...
            if ( var_2.team == "allies" )
                var_0++;
            else if ( var_2.team == "axis" )
                var_1++;
        }

        level.grnd_numplayers[ "allies" ] = var_0;
        level.grnd_numplayers[ "axis" ] = var_1;

        wait 0.5;
    }
}

// Re-export of the stock grnd helper the bots mod (_bot_script.gsc) links against;
// the decompiled source dropped it, which broke LinkFile -> server crash-loop.
isingrindzone()
{
    if ( isdefined( self.ingrindzone ) )
        return self.ingrindzone;
