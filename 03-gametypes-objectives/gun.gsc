...
main()
{
    ...
    // yourserver.gg: force random gun order whether or not match-rules data is present
    level.matchrules_randomize = 1;
...
{
    // yourserver.gg: an akimbo pool entry carries a unique key (gg_akimbo_*); map it back to the
    // real base weapon. Normal entries (key == base) have no gun_base entry and pass through.
    if ( isdefined( level.gun_base ) && isdefined( level.gun_base[var_0] ) )
        basewep = level.gun_base[var_0];
    else
        basewep = var_0;
    ...
    var_3 = maps\mp\gametypes\_class::buildweaponname( basewep, var_1, var_2, 0, 0 );
    return var_3;
}
...
setguns()
{
    // bpg 2026-07-27: the ladder is now DRAWN from this pool rather than being it.
    // Every match shuffles the pool and takes 30, so the rungs differ game to game while
    // the win condition stays 30 - scr_gun_scorelimit is set from level.gun_guns.size
    // higher up in this file, so it follows the draw automatically and needs no dvar edit.
    //
    // 17 entries were REMOVED here as the cause of "new guns show invisible hands": they
    // named weapons with no definition anywhere on the server (verified against the
    // custom mod.ff zone, mod.iwd, the loose weapons folder and the localized zone).
    //   iw5_iw4coltanaconda / iw5_iw4wa2000 - mod.ff ships 15 iw4 ports, not 17
    //   gg_akimbo_* (15) - pseudo-keys resolved by buildweaponname() to base+akimbo;
    //                     those variants do not exist, and akimbo was already reverted
    //                     2026-07-18 for causing deterministic client crashes.
    // A weapon token the engine cannot resolve is exactly what renders as empty hands,
    // so every token below is one whose definition was confirmed present.
    level.gun_pool = [];

    // stock IW5 weapons - no launchers/riotshield, so a rung cannot stall on a weapon
    // that never gets kills
    level.gun_pool[0] = "iw5_deserteagle";
    level.gun_pool[1] = "iw5_44magnum";
    level.gun_pool[2] = "iw5_g18";
    level.gun_pool[3] = "iw5_fmg9";
    level.gun_pool[4] = "iw5_mp9";
    level.gun_pool[5] = "iw5_skorpion";
    level.gun_pool[6] = "iw5_mp5";
    level.gun_pool[7] = "iw5_p90";
    level.gun_pool[8] = "iw5_mp7";
    level.gun_pool[9] = "iw5_ump45";
    level.gun_pool[10] = "iw5_pp90m1";
    level.gun_pool[11] = "iw5_m9";
    level.gun_pool[12] = "iw5_m4";
    level.gun_pool[13] = "iw5_m16";
    level.gun_pool[14] = "iw5_ak47";
    level.gun_pool[15] = "iw5_scar";
    level.gun_pool[16] = "iw5_acr";
    level.gun_pool[17] = "iw5_type95";
    level.gun_pool[18] = "iw5_fad";
    level.gun_pool[19] = "iw5_g36c";
    level.gun_pool[20] = "iw5_cm901";
    level.gun_pool[21] = "iw5_mk14";
    level.gun_pool[22] = "iw5_pecheneg";
    level.gun_pool[23] = "iw5_mg36";
    level.gun_pool[24] = "iw5_m60";
    level.gun_pool[25] = "iw5_mk46";
    level.gun_pool[26] = "iw5_spas12";
    level.gun_pool[27] = "iw5_1887";
    level.gun_pool[28] = "iw5_barrett";
    level.gun_pool[29] = "iw5_msr";

    // MW2 (iw4) ports - each one's weapon definition was confirmed inside the custom weapon mod.ff
    level.gun_pool[30] = "iw5_iw4aug";
    level.gun_pool[31] = "iw5_iw4beretta";
    level.gun_pool[32] = "iw5_iw4beretta393";
    level.gun_pool[33] = "iw5_iw4fal";
    level.gun_pool[34] = "iw5_iw4famas";
    level.gun_pool[35] = "iw5_iw4fn2000";
    level.gun_pool[36] = "iw5_iw4kriss";
    level.gun_pool[37] = "iw5_iw4m1014";
    level.gun_pool[38] = "iw5_iw4m240";
    level.gun_pool[39] = "iw5_iw4mp5k";
    level.gun_pool[40] = "iw5_iw4pp2000";
    level.gun_pool[41] = "iw5_iw4ranger";
    level.gun_pool[42] = "iw5_iw4rpd";
    level.gun_pool[43] = "iw5_iw4tavor";
    level.gun_pool[44] = "iw5_iw4uzi";

    // draw THIS match's 30-rung ladder
    level.gun_guns = bpg_gun_draw_ladder( level.gun_pool, 30 );
...
// bpg 2026-07-27: Fisher-Yates over a COPY of the pool, then take the first $count.
// Copying keeps level.gun_pool intact for the next map, and drawing without replacement
// guarantees all 30 rungs are different weapons.
bpg_gun_draw_ladder( pool, count )
{
    bag = [];
